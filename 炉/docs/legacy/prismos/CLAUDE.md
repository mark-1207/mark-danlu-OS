...
> 版本：v1.4.0 | 创建：2026-06-05
> 权威：与 `skills/prism-os/CLAUDE.md` 内容相同；流程规范以 `skills/prism-os/SKILL.md` 为准
>
> **同步声明**：本文件是 `skills/prism-os/CLAUDE.md` 的**副本**（根级 fallback）。主源是 skill 级版本。任何修改必须两处同步。
...
| 5 | 跑 `run` 后不问用户就跑 M3b 系列模式 | `--series` 标志必须显式传；不传就跑默认流程，不擅自追加 |
| 6 | 用户陈述独立观点时不识别为选题 | 用 3 场景决策树判断：创作动词 → 直接跑；回顾词 → 列库存；独立观点+张力 → offer y/n。触发词库见 `memory/trigger_word_library.md`，决策树见 `SKILL.md § 8` |
