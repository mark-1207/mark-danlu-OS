...
# ============ A5: CCOS prompt 优化（反面+正面示例）============

class TestCCOSFlowExamples(unittest.TestCase):
    """A5: CCOS 反面+正面 模块流示例"""

    def test_negative_examples_at_least_3(self):
        from cognitive_outline import CCOS_NEGATIVE_FLOW_EXAMPLES
        self.assertGreaterEqual(
            len(CCOS_NEGATIVE_FLOW_EXAMPLES), 3,
            f"CCOS 反面示例应至少 3 条，当前 {len(CCOS_NEGATIVE_FLOW_EXAMPLES)}"
        )

    def test_positive_examples_at_least_3(self):
        from cognitive_outline import CCOS_POSITIVE_FLOW_EXAMPLES
        self.assertGreaterEqual(
            len(CCOS_POSITIVE_FLOW_EXAMPLES), 3,
            f"CCOS 正面示例应至少 3 条，当前 {len(CCOS_POSITIVE_FLOW_EXAMPLES)}"
        )

    def test_each_example_is_tuple_of_two_strings(self):
        from cognitive_outline import (
            CCOS_NEGATIVE_FLOW_EXAMPLES,
            CCOS_POSITIVE_FLOW_EXAMPLES,
        )
        all_ex = list(CCOS_NEGATIVE_FLOW_EXAMPLES) + list(CCOS_POSITIVE_FLOW_EXAMPLES)
        for ex in all_ex:
            self.assertEqual(len(ex), 2, f"示例应为 (flow, reason) 二元组: {ex}")
            self.assertIsInstance(ex[0], str)
            self.assertTrue(len(ex[0]) > 0, f"flow 为空: {ex}")
            self.assertIsInstance(ex[1], str)
            self.assertTrue(len(ex[1]) > 0, f"reason 为空: {ex}")

    def test_format_helper_returns_block(self):
        from cognitive_outline import _format_ccos_flow_examples
        block = _format_ccos_flow_examples()
    ...
    def test_module_flow_prompt_embeds_examples(self):
        """验证 generate_cognitive_module_flow 的 prompt 包含示例块"""
        ...
        original = cognitive_outline._call_llm_raw
        cognitive_outline._call_llm_raw = fake
        try:
            generate_cognitive_module_flow(
                "测试命题", "认知升级型",
                {"认知倾向": "分析优先", "表达气质": "理性深刻",
            ...
            cognitive_outline._call_llm_raw = original

        self.assertEqual(len(captured), 1)
        prompt = captured[0]
        # prompt 应嵌入格式化后的示例块（检查前 30 字符）
        block = _format_ccos_flow_examples()
        self.assertIn(block[:30], prompt, "module_flow prompt 应嵌入 _format_ccos_flow_examples() 输出")
