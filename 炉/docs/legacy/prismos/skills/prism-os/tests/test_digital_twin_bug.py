...
def test_twin_phase_returns_rejected_topics():
    ...
    with patch("cognitive_crack.learn_and_filter_combined") as mock_combined:
        mock_combined.return_value = {
            ...
            "rejected_topics": [{"topic": "T2", "rejection_reason": "Y"}],
...
def test_twin_phase_panic_on_error():
    """panic_on_error=True 时，learn_and_filter_combined 失败应 raise"""
    from phases.twin import TwinPhase
    from phases.base import PipelineState, PipelineConfig
    ...
    # v1.1: 模拟 learn_and_filter_combined 失败
    import cognitive_crack
    original = cognitive_crack.learn_and_filter_combined
    cognitive_crack.learn_and_filter_combined = lambda *a, **kw: (_ for _ in ()).throw(RuntimeError("mock failure"))
    try:
        with pytest.raises(RuntimeError, match="mock failure"):
            phase.execute(state, config)
    finally:
        cognitive_crack.learn_and_filter_combined = original
