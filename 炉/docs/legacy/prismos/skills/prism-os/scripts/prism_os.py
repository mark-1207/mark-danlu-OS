...
def run_prism_os(
    ...
    Args:
        user_input: 用户输入
        identity_role: 用户身份（可选，CLI 覆盖 persona 中的 identity_role）
        audience: 目标受众（可选，CLI 覆盖 persona 中的 audience）
        include_phase_4_8: 是否包含 Phase 4-8（默认 True）
        materials: 现有素材（可选）
        history_topics: 历史选题列表（可选）
        skip_gateway: 跳过 Phase 1 熵值判断（默认 False）
        persona_name: 人设名（默认 "default"，对应 data/personas.yaml 中 default 段）
    ...
    # M1: 设置模块级 persona_name（被 cognitive_outline._load_authorial_identity 读取）
    try:
        import cognitive_outline as _co_module
        _co_module._PERSONA_NAME = persona_name
    except Exception:
        pass

    # dry-run: mock LLM 调用，所有 Phase 走 fallback 路径
    if dry_run:
        import call_llm as _call_llm_module
        _original_call_llm_raw = _call_llm_module.call_llm_raw
        _original_call_llm = _call_llm_module.call_llm
        _call_llm_module.call_llm_raw = lambda prompt, **kwargs: None
        _call_llm_module.call_llm = lambda prompt, **kwargs: {"content": "", "error": "dry-run"}
        print("[DRY-RUN] 模拟运行：不调用 LLM，各 Phase 使用 fallback 值", file=sys.stderr)
    ...
    # dry-run: 恢复原始 LLM 函数
    if dry_run:
        _call_llm_module.call_llm_raw = _original_call_llm_raw
        _call_llm_module.call_llm = _original_call_llm
...
def _run_narrate(topic: str, platform: str, style: dict = None) -> dict:
    """内部 narrate 调用：加载 CCOS → 注入 persona 风格 → 生成内容 → 落盘文件。失败抛异常。

    Args:
        topic: 文章主题
        platform: wechat / xiaohongshu
        style: M1 persona 风格（tone_keywords/avoid_keywords/style_keywords/platform_preference）
    """
...
def _run_series_workflow(user_input: str, result: Dict, action: Optional[str],
                         interactive: bool) -> None:
    """
    系列模式入口：run 成功后调用。
    - 锚点 = 当前 run 的 T1
    - action=None → 交互式（除非 interactive=False 则走 save-all）
    - action="no" → 跳过
    - action="yes"/"save-all" → 自动 save-all（无 prompt）
    - action="auto" → 同 save-all
    """
    from series import (
        analyze_suitability, generate_series_titles,
        handle_series_command, make_series_id,
    )
    ...
    # 1. 提取锚点
    anchor_title = ""
    sel = result.get("selected_candidate")
    if isinstance(sel, dict):
        anchor_title = sel.get("title", "") or ""
    if not anchor_title:
        anchor_title = user_input
    ...
    # 3. 适配性分析
    print("\n━━━ 系列模式（M3b）━━━", file=sys.stderr)
    print(f"  锚点: {anchor_title[:40]}", file=sys.stderr)

    suitability = analyze_suitability(user_input, persona)

    if not suitability.get("suitable"):
        reason = suitability.get("primary_reason", "不适合系列展开")
        print(f"  ⊘ 不适合系列: {reason}", file=sys.stderr)
        print("  → 锚点 T1 已保留，不展开系列", file=sys.stderr)
        result["series"] = {
            "status": "skipped",
            "reason": reason,
    ...
    reason = suitability.get("primary_reason", "话题可展开")
    confidence = suitability.get("confidence", 0)
    print(f"  ✓ 适合系列: {reason} (置信度 {confidence:.2f})", file=sys.stderr)

    # 4. action 决策
    if action == "no":
        print("  → 跳过系列规划（--series-action no）", file=sys.stderr)
        result["series"] = {
            "status": "skipped",
    ...
    auto_save_all = (action in ("yes", "save-all", "auto")) or (
        action is None and not interactive
    )

    # 5. 生成 3 个标题
    print("  生成系列标题中...", file=sys.stderr)
    series_titles = generate_series_titles(
        anchor_title=anchor_title,
        anchor_thesis=user_input,
        anchor_depth=None,
        persona=persona,
        count=3,
    )
    ...
    if len(series_titles) < 1:
        print("  ⚠ 系列标题生成失败（无有效候选）", file=sys.stderr)
        result["series"] = {
            "status": "failed",
            "reason": "系列标题生成失败",
            "suitability": suitability,
    ...
    if len(series_titles) < 3:
        print(f"  ⚠ 仅生成 {len(series_titles)} 个有效标题（张力不足）", file=sys.stderr)

    series_id = make_series_id(user_input)
    state = {"series_id": series_id, "titles": series_titles}

    if auto_save_all:
        chosen = list(range(len(series_titles)))
        action_result = {"action": "save_all"}
    else:
        # 交互式 prompt
        print(f"  series_id: {series_id}", file=sys.stderr)
        print("\n  系列候选标题:", file=sys.stderr)
        for i, t in enumerate(series_titles, 1):
            pos = t.get("positioning", "?")
            score = t.get("tension_score", "?")
            print(f"    {i}. [{pos}|tension={score}] {t.get('title', '')}",
                  file=sys.stderr)
        print("  操作: 1-3 选一个 / s 全部保存 / m 重新生成 / q 跳过",
              file=sys.stderr)
        ...
        while True:
            ...
            action_result = handle_series_command(user_cmd, state)
            if action_result.get("action") == "error":
                print(f"  ✗ {action_result.get('message')}", file=sys.stderr)
                continue
            break

        if action_result.get("action") == "skip":
            print("  → 跳过系列规划", file=sys.stderr)
            result["series"] = {
                "status": "skipped",
                "reason": "user skipped",
                "suitability": suitability,
        ...
        if action_result.get("action") == "regenerate":
            print("  重新生成中...", file=sys.stderr)
            series_titles = generate_series_titles(
                anchor_title=anchor_title,
                anchor_thesis=user_input,
                anchor_depth=None,
            ...
            if not series_titles:
                ...
                return
            state["titles"] = series_titles
            action_result = {"action": "save_all"}

        if action_result.get("action") == "select":
            chosen = [action_result["index"]]
        elif action_result.get("action") == "save_all":
            chosen = list(range(len(series_titles)))
        else:
            chosen = []

    # 6. 写 topic_log
    planned = [{"slot": 1, "title": anchor_title, "status": "anchor"}]
    for i, t in enumerate(series_titles, 2):
        planned.append({
            "slot": i,
            "title": t.get("title", ""),
    ...
    log_entry = {
        ...
        "series": {
            ...
            "chosen_slots": [i + 1 for i in chosen_set],  # 1-indexed
        },
    }

    append_log(log_entry)

    print(f"\n  ✓ 系列已记录: {series_id}", file=sys.stderr)
    print(f"    planned: {len(planned)} 个标题", file=sys.stderr)
    if chosen_set:
        chosen_titles = [planned[i + 1]["title"][:30] for i in chosen_set]
        print(f"    用户选中: {chosen_titles}", file=sys.stderr)
    print("  后续跑 T2/T3: python prism_os.py run \"<T 命题>\"", file=sys.stderr)
    print(f"  写完标记: python prism_os.py mark_written --series-id {series_id} --slot <2|3>",
          file=sys.stderr)
    ...
    result["series"] = {
        ...
        "chosen_slots": [i + 1 for i in chosen_set],
    }
...
def main():
    if len(sys.argv) < 2:
        _safe_print({
            ...
            "commands": {
                ...
                "mark_written": "python prism_os.py mark_written --series-id <sid> --slot <n> - 标记系列文章已写完"
            },
            "options": {
                "--format, -f": "格式化输出（可读报告）",
                ...
                "--series": "M3b 系列模式：锚点跑完后规划 3 篇后续标题",
                "--series-action <yes|no|save-all>": "系列模式动作（默认交互式）"
            }
        })
        sys.exit(1)
    ...
    # --help/-h: 显示帮助并退出（不触发 LLM）
    if command in ("--help", "-h", "help"):
        _safe_print({
            "commands": {
                "run": "python prism_os.py run \"<用户输入>\" [--format] [--no-ext] - 完整流程",
                "classify": "Phase 0: 意图识别",
                "gateway": "Phase 1: 苏格拉底网关（熵值计算）",
                "prism": "Phase 2: 棱镜引擎（生成正交标题候选）",
                "anchor": "Phase 3: 现实校验锚（验证候选标题）",
            ...
            "options": {
                "--format, -f": "格式化输出（可读报告）",
                "--no-ext": "跳过 Phase 4-8（仅 Phase 0-3）",
                "--no-interactive": "跳过人工决策点（默认选第一个）",
                "--skip-gateway": "跳过 Phase 1 苏格拉底网关（调试用）",
                "--no-ccos-review": "跳过 CCOS 大纲人工审核",
                "--from-queue": "从 crack_queue 选择裂缝进入主流程",
                "--match-queue": "输入时匹配 crack_queue 中的相关裂缝",
                "--dry-run": "模拟运行（不调 LLM，输出流程结构）",
                "--series": "M3b 系列模式：锚点跑完后规划 3 篇后续标题",
                "--series-action <yes|no|save-all>": "系列模式动作（默认交互式）"
            }
        })
    ...
    known_commands = {"run", "classify", "gateway", "prism", "anchor", "twin", "gap", "logic", "save", "assassin", "confirm", "ccos", "generate", "narrate", "queue", "archive", "metrics", "mark_written"}
    ...
    if command == "run":
        ...
        run_dry_run = False
        run_series = False
        run_series_action = None  # None | "yes" | "no" | "save-all"
        ...
        while i < len(sys.argv):
            ...
            if arg == "--format" or arg == "-f":
                ...
                run_series = True
                i += 1
            elif arg == "--series-action" and i + 1 < len(sys.argv):
                run_series_action = sys.argv[i + 1].lower()
                run_series = True
                i += 2
        ...
        # v1.4: need_input 时自动读 stdin 循环 resume
        while result.get("status") == "need_input" and run_interactive:
            prompt_text = result.get("_prompt", "")
            if prompt_text:
        ...
        # M3b: --series 触发系列模式（narrate 之后）
        if run_series and result.get("status") == "success":
            try:
                _run_series_workflow(
                    user_input, result,
                    action=run_series_action,
                    interactive=run_interactive,
                )
            except Exception as e:
                print(f"[WARNING] 系列模式失败（run 状态保留）: {e}", file=sys.stderr)
                result["series"] = {"status": "failed", "error": str(e)}
    ...
    elif command == "mark_written":
        # M3b: 标记 series 的指定 slot 已写完
        # 用法: python prism_os.py mark_written --series-id <sid> --slot <n>
        series_id = None
        slot = None
        i = 2
        while i < len(sys.argv):
            arg = sys.argv[i]
            if arg == "--series-id" and i + 1 < len(sys.argv):
                series_id = sys.argv[i + 1]
                i += 2
            elif arg == "--slot" and i + 1 < len(sys.argv):
                try:
                    slot = int(sys.argv[i + 1])
                except ValueError:
                    _safe_print({"error": f"--slot 必须是整数: {sys.argv[i + 1]}"})
                    sys.exit(1)
                i += 2
            else:
                i += 1
        ...
        if not series_id or slot is None:
            _safe_print({
                "error": "用法: python prism_os.py mark_written --series-id <sid> --slot <n>"
            })
            sys.exit(1)

        from storage import mark_series_written
        try:
            result = mark_series_written(series_id, slot)
            _safe_print(result)
        except Exception as e:
            _safe_print({"status": "error", "message": str(e)})
            sys.exit(1)
