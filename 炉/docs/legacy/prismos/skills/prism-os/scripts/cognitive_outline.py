...
def _load_authorial_identity(*args, **kwargs) -> Dict:
    """
    M1 改造：优先从 persona 加载人设，缺字段时回退到 LLM 数字分身提取。
    ...
    返回 Layer 7 注入格式：
    {
        ...
        "价值倾向": str,
        "长期母题": str,
        "thinking_pattern": dict,        # 来自 LLM（动态）
        "dimension_weights": dict,       # 来自 LLM（动态，persona 不管这个）
        "style_keywords": List[str],     # 来自 persona（覆盖 LLM）
        "audience": str,                # 来自 persona
    }

    M1 兼容性：接受任意参数（被 lambda mock 替换时也能调用）。
    """
    # 从 kwargs 提取 persona_name（如果传了）
    persona_name = kwargs.get("persona_name", _PERSONA_NAME)
    ...
    try:
        ...
        from persona import load as _load_persona, format_for_ccos
        persona = _load_persona(persona_name)
    except Exception as e:
        print(f"[Warning] 加载 persona 失败: {e}", file=sys.stderr)

    # 2) LLM 提取 thinking_pattern + dimension_weights（persona 不管这些）
    pattern_result = {}
    try:
        sys.path.insert(0, str(Path(__file__).parent))
    ...
    # 3) 合并：persona 字段覆盖 LLM 字段
    if persona:
        mapped = format_for_ccos(persona)  # {认知倾向, 表达气质, 价值倾向, 长期母题}
        ...
        try:
            ...
            twin_result = digital_twin_filter([], thinking_pattern)
            audience = twin_result.get("audience", "")
        except Exception:
            audience = ""
        style_keywords = pattern_result.get("style_keywords", [])
        # 旧 authorial_identity 字段
        mapped = {
            "认知倾向": thinking_pattern.get("倾向", "分析优先") if isinstance(thinking_pattern, dict) else (thinking_pattern or "分析优先"),
            "表达气质": thinking_pattern.get("气质", "理性深刻") if isinstance(thinking_pattern, dict) else "理性深刻",
...
# M1: 模块级 persona_name（run_prism_os 可在调用前设置）
...
def extract_cognitive_tension(topic: str) -> Dict:
    """M8 改造：返回认知张力 + 4 层深度展开（一次 LLM 调用，0 新增）"""
...
"""
    raw = _call_llm_raw(prompt)
    if not raw:
        return _default_cognitive_tension()
    parsed = _parse_llm_json(raw)
    if not parsed:
        return _default_cognitive_tension()
    return parsed
...
def _default_cognitive_tension() -> Dict:
    return {
        "认知张力": {"大众以为": "应该跟随大众", "现实是": "独立思考才能突破"},
        "深度展开": {
            "surface": "",
            "implication": "",
            "deeper_meaning": "",
...
def get_depth_expansion_from_tension(tension_result: Dict) -> Dict:
    """从认知张力的 LLM 输出中提取 4 层深度展开（M8 配套）"""
    if not isinstance(tension_result, dict):
        return {}
    return tension_result.get("深度展开", {}) or {}
...
# ============ A5: CCOS 反面+正面 模块流示例 ============
# 用于 generate_cognitive_module_flow 的 prompt，让 LLM 看到"应该/不应该"的模块流范式

CCOS_NEGATIVE_FLOW_EXAMPLES = [
    (
        "HOOK('AI 改变世界') → EXPLAIN('深度分析') → MODEL('认知框架') → EVIDENCE('99% 数据')",
        "全是抽象描述，读者无代入感（无 CASE、无具体人称）"
    ),
    (
        "HOOK('AI 时代你必须知道的 3 件事') → EXPLAIN('观点 1') → EXPLAIN('观点 2') → EXPLAIN('观点 3')",
        "说教式开头命中 BANNED_WORDS 模板，无反直觉点"
    ),
    (
        "EVIDENCE('统计 1') → EVIDENCE('统计 2') → EVIDENCE('统计 3') → EVIDENCE('统计 4')",
        "连续 4 个数据无解读，读者看完无感（缺 EXPLAIN / COUNTER）"
    ),
    (
        "MODEL('AI 的本质是认知革命') → MODEL('底层逻辑深度解读') → EXPLAIN",
        "伪深度，命中 pseudo_depth 类禁用词，无具体场景支撑"
    ),
    (
        "EXPLAIN → MODEL → EVIDENCE → ACTION（全程正向论证）",
        "缺 COUNTER 反直觉点，文章读完无记忆点"
    ),
]

CCOS_POSITIVE_FLOW_EXAMPLES = [
    (
        "HOOK('凌晨 3 点我让 ChatGPT 帮我写辞职信') → CASE('讲那天具体场景') → COUNTER('老板没生气回了一句话')",
        "具体时间 + 第一人称 + 反转，HOOK 与 CASE 一体"
    ),
    (
        "HOOK('公司开 AI 分享会全场只来 2 个人') → CASE('那 2 人的反应') → COUNTER('他们不是不想学，是已经被裁了')",
        "反差画面 + 人物反应 + 反直觉结论"
    ),
    (
        "MODEL('提问漏斗：场景→约束→反例→反直觉') → EXPLAIN → CASE(用一个真实咨询案例走一遍)",
        "可复用框架 + 具体案例，密度高"
    ),
    (
        "观点主体 → COUNTER('但这招只对内容创作者有效') → BOUNDARY('对程序员/销售可能不适用')",
        "主动声明适用边界，高级感"
    ),
    (
        "EXPLAIN → MODEL → ACTION('第一步：关掉所有推送；第二步：每天写 3 条反常识卡')",
        "步骤化可执行，落地明确"
    ),
]


def _format_ccos_flow_examples() -> str:
    """把 CCOS 反面+正面 模块流示例格式化为 prompt 片段。"""
    negative_block = "\n".join(
        f"  - ❌ {flow}\n    （问题：{reason}）"
        for flow, reason in CCOS_NEGATIVE_FLOW_EXAMPLES
    )
    positive_block = "\n".join(
        f"  - ✓ {flow}\n    （技巧：{reason}）"
        for flow, reason in CCOS_POSITIVE_FLOW_EXAMPLES
    )
    return (
        f"**反面示例（避免这类模块流）：**\n{negative_block}\n\n"
        f"**正面示例（参考这类模块流）：**\n{positive_block}"
...
def generate_cognitive_module_flow(topic: str, structure: str, authorial_identity: Dict, platform: str) -> List[Dict]:
    """生成认知模块流"""
    examples_block = _format_ccos_flow_examples()
...
# v1.1: 合并 Layer 4 (模块流) + Layer 8 (势能曲线) 为 1 次 LLM 调用
# 节省：1 call per 平台 = 2 calls (both 平台)
# 风险：低 — 两者输入（topic+structure）相同，输出正交（结构 vs 节奏）
# 缓解：parse 时容错，任意一段缺失则用 rule-based fallback


def generate_module_flow_and_energy(
    topic: str,
    structure: str,
    authorial_identity: Dict,
    platform: str,
) -> Dict:
    """
    1 次 LLM 调用同时产出：模块流（Layer 4）+ 势能曲线（Layer 8）。

    Returns:
        {
            "module_flow": List[Dict],
            "narrative_energy": Dict,
        }

    容错：若 LLM 漏掉任一段，单独 fallback 调用对应单段函数。
    """
    examples_block = _format_ccos_flow_examples()
    prompt = f"""根据命题和结构，同时设计认知模块流（Layer 4）和内容势能曲线（Layer 8）。
...
{{
  ...
  "narrative_energy": {{
    ...
    # 兜底：完整 fallback
    fallback_module_flow = generate_cognitive_module_flow(topic, structure, authorial_identity, platform)
    fallback_energy = generate_narrative_energy(topic, fallback_module_flow, platform)

    if not raw:
        return {"module_flow": fallback_module_flow, "narrative_energy": fallback_energy}

    parsed = _parse_llm_json(raw)
    if not parsed:
        return {"module_flow": fallback_module_flow, "narrative_energy": fallback_energy}

    # 任意一段缺失就 fallback 那一段
    result_module_flow = parsed.get("module_flow") if isinstance(parsed.get("module_flow"), list) else None
    result_energy = parsed.get("narrative_energy") if isinstance(parsed.get("narrative_energy"), dict) else None

    if not result_module_flow:
        result_module_flow = generate_cognitive_module_flow(topic, structure, authorial_identity, platform)
    if not result_energy:
        result_energy = generate_narrative_energy(topic, result_module_flow, platform)

    return {"module_flow": result_module_flow, "narrative_energy": result_energy}
...
def cognitive_outline_workflow(
    ...
    # M1: 优先用 persona 字段（_PERSONA_NAME 模块级变量可被 run_prism_os 覆盖）
    ...
    # v1.1: Layer 4 (模块流) + Layer 8 (势能曲线) 合并为 1 次 LLM 调用
    flow_and_energy = generate_module_flow_and_energy(
        topic, main_structure, authorial_identity, platform
    )
    module_flow = flow_and_energy["module_flow"]
    narrative_energy = flow_and_energy["narrative_energy"]
