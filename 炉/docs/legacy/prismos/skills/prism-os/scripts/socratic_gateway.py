...
# v1.1: 方向差异说明（LLM 生成，1 次调用 per run）
DIRECTION_DIFFERENCE_PROMPT = """为给定的多个方向生成简短的"差异说明"，帮用户做选择。
...
# 输出（严格 JSON 数组，与方向顺序一致）
[
  "方向1的差异说明（一句话，强调这个方向切入点的独特性/对比其他方向的区别）",
  "方向2的差异说明",
  ...
]
...
要求：
- 每句 ≤ 30 字
- 重点说明"这个方向跟其他方向相比，独特点在哪"
- 不要重复方向本身的文本
- 用对比性语言（"侧重 X 视角"、"避开 Y 共识"、"强调 Z 张力"）"""


def generate_direction_differences(thesis: str, directions: List[str]) -> List[str]:
    """
    为每个方向生成一句"差异说明"，帮用户做选择。

    Returns:
        与 directions 等长的 List[str]。失败时返回空列表（调用方应降级为不加说明）。
    """
    if not directions:
        return []
    directions_block = "\n".join([f"{i+1}. {d}" for i, d in enumerate(directions)])
    prompt = DIRECTION_DIFFERENCE_PROMPT.format(
        thesis=thesis,
    ...
    if not parsed or not isinstance(parsed, list):
        return []
    diffs = [d for d in parsed if isinstance(d, str) and d.strip()]
    return diffs[: len(directions)]
