...
# ============ v1.1: Twin learn + filter 合并 ============

def learn_and_filter_combined(
    candidates: List[Dict],
    history_limit: int = 50,
) -> Dict:
    """
    1 次 LLM 调用同时完成：学习创作者思维特征 + 筛选候选。

    取代原 pipeline 中 learn_thinking_pattern + digital_twin_filter 的两次调用。
    ...
    Returns:
        {
            ...
            "confidence": float,
            "growth_stage": str,
            "sensitive_directions": List[str],
            "worldview": str,
            "selected_topics": List[Dict],
            "rejected_topics": List[Dict],
            "digital_twin_confidence": float,
        }
    """
    sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
    ...
    history = load_log(limit=history_limit)
    thesis_sample = [e.get("thesis", "") for e in history if e.get("thesis")][:10]
    thesis_str = "\n".join([f"- {t}" for t in thesis_sample])
    ...
    prompt = f"""分析创作者思维特征，并基于特征筛选候选选题（一次完成两件事）。
...
分析：
- dimension_weights: reversal（逆向拆解）/ micro_scene（微观切片）/ systemic_flaw（系统归因）/ bridge（认知脚手架）的权重（0-1）
- style_keywords: 常用词汇
- thinking_pattern: 3-5 词概括风格
- growth_stage: 工具型/方法型/系统型/思想型
- sensitive_directions: 敏感主题列表
- worldview: 一句话世界观
...
按创作者思维特征筛 candidate，保留有"认知落差"的，过滤模式化的。
...
{{
  ...
  "sensitive_directions": ["..."],
  "worldview": "...",
  "selected_topics": [{{"topic": "...", "selection_reason": "...", "confidence": 0.0-1.0}}],
  "rejected_topics": [{{"topic": "...", "rejection_reason": "...", "confidence": 0.0-1.0}}],
  "digital_twin_confidence": 0.0-1.0
}}"""
    ...
    parsed.setdefault("thinking_pattern", "")
    parsed.setdefault("dimension_weights", {})
    parsed.setdefault("style_keywords", [])
    parsed.setdefault("selected_topics", [])
    parsed.setdefault("rejected_topics", [])
    parsed.setdefault("digital_twin_confidence", 0.0)
    parsed.setdefault("growth_stage", "方法型")
    parsed.setdefault("sensitive_directions", [])
    parsed.setdefault("worldview", "")
    parsed["confidence"] = min(1.0, len(thesis_sample) / 20)
    return parsed
