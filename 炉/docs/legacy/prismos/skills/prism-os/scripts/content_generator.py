...
def narrative_generation_workflow(
    ...
    # Step 4: 一稿生成（v1.1 优化：删 prenarrative + 删 expansion）
    # prenarrative 在标准 pipeline 中不显示，直接进 full_draft（保留函数供 interactive 使用）
    prenarrative = ""  # 跳过 generate_prenarrative 调用
    result["prenarrative"] = ""
    ...
    # Step 5b: 字数校验（v1.1 优化：删 expansion，靠 prompt 约束 + writing-cn scene 的 16384 max_tokens）
...
def quality_check(article_text: str, platform: str, persona: Dict = None) -> Dict:
    """
    写作后质量自检：L1自动拦截 + L2-L4 AI腔 LLM审核 + L5 风格指纹 + L6 密度

    Args:
        article_text: 文章全文
        platform: wechat / xiaohongshu
        persona: 人设 dict（可选，提供时启用 L5/L6）

    Returns:
        {
            "status": "pass" | "issues_found" | "check_failed",
            "platform": str,
            "issues": [{"level": "L1"|"L2"|"L3"|"L4"|"L5"|"L6", "type": str, "location": str, "suggestion": str, "severity": "error"|"warning"}],
            "ai_mannerisms": [{"type": str, "count": int, "examples": [str]}],
            "score": float,  # 0-100
            "style_check": {...},   # L5（仅当 persona 提供时）
            "density_check": {...},  # L6（仅当 persona 提供时）
        }
    """
        ...
        if parsed:
            ...
            if persona:
                try:
                    ...
                    if style_check.get("anti_issues"):
                        for kw in style_check["anti_issues"]:
                            issues.append({
                                "level": "L5",
                                "type": "风格违反",
                                "location": "全文",
                                "suggestion": f"避免使用'{kw}'（属于 avoid_keywords）",
                                "severity": "warning",
                            })
                except Exception:
                    pass

            # L6: 信息密度检查（1 次 LLM，需 persona）
            density_check = None
            if persona:
                try:
                    from density import measure_density, check_l6_density
                    density_report = measure_density(article_text, persona)
                    density_check = check_l6_density(density_report)
                    if density_check.get("suggestions"):
                        for s in density_check["suggestions"]:
                            issues.append({
                                "level": "L6",
                                "type": "密度建议",
                                "location": "全文",
            ...
            if style_check is not None:
                result["style_check"] = style_check
            if density_check is not None:
                result["density_check"] = density_check
            return result
