...
def append_log(entry: Dict) -> Dict:
    """追加选题日志。v1.1: 同时存 _title_embedding（A3 风格一致性用）。"""
    log_path = os.path.join(get_data_dir(), "topic_log.yaml")
    logs = load_yaml(log_path)

    # v1.1 M6: 存 embedding，避免 A3 每次重算
    selected_title = entry.get("selected_title")
    if selected_title:
        try:
            from embedding import embed
            title_vec = embed(selected_title)
            if title_vec is not None:
                entry["_title_embedding"] = list(title_vec)
        except Exception:
            pass  # embedding 失败不阻断存储
...
def mark_series_written(series_id: str, slot: int) -> Dict:
    """
    把 series 的指定 slot 标记为 written。
    全部 slot 都 written 时整个 series.status → complete，否则 partial。

    Args:
        series_id: 系列 id（series-YYYYMMDD-<slug>-<6位 hex>）
        slot: 要标记的 slot 编号（1=anchor，2/3=后续）

    Returns:
        {"status": "ok", "series_status": "partial"|"complete", "slot": int}

    Raises:
        PRISMError: series_id 不存在或 slot 越界
    """
    log_path = os.path.join(get_data_dir(), "topic_log.yaml")
    logs = load_yaml(log_path)
    ...
    if target_entry is None:
        raise PRISMError(f"series_id 不存在: {series_id}")

    series = target_entry["series"]
    planned = series.get("planned_titles", [])
    slot_found = False
    for p in planned:
        if p.get("slot") == slot:
            p["status"] = "written"
    ...
    if not slot_found:
        raise PRISMError(f"slot {slot} 不存在于 series {series_id}")

    # 整 series 状态
    all_written = planned and all(
        p.get("status") in ("written", "anchor") for p in planned
    )
    series["status"] = "complete" if all_written else "partial"
    ...
    raise PRISMError("保存 series 状态失败")
