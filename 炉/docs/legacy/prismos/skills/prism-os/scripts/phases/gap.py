...
class GapPhase(Phase):
    ...
    def _format_gap_prompt(self, gap_result: dict, threshold: float = 0.3) -> str:
        """格式化 Gap 决策提示（v1.1: 加 ⭐ 推荐）"""
        score = gap_result.get("gap_score", 0)
        readiness = gap_result.get("readiness", 0)
        missing = gap_result.get("missing_evidence", [])
        # v1.1: ⭐ 推荐规则
        # readiness ≥ 70% → [3] 直接生成（就绪度高，材料够用）
        # readiness < 70% → [2] 调整大纲（先优化大纲再补素材更高效）
        rec_opt = "3" if readiness >= 0.7 else "2"
        star = f" ⭐（推荐选 {rec_opt}）" if rec_opt == "2" else f" ⭐（推荐选 {rec_opt}，就绪度高）"
        lines = [
            "\n━━━ Gap 决策 ━━━",
        ...
        lines.extend([
            "━━━━━━━━━━━━━━━━━━━━",
            f"请选择: [1] 补充素材 → 输入素材内容，重新评估后再次决策",
            f"  [2] 调整大纲{star} → 保留标题，重新生成 CCOS 大纲（约 10-30 秒）",
            f"  [3] 直接生成 → 跳过素材补充，直接写文章（⚠ 就绪度 {readiness:.0%}）",
            f"  [q] 退出 → 不保存（可重新跑 run 再来）",
            f"  其他输入 → 重新显示本提示",
        ])
        return "\n".join(lines)
    ...
    def _handle_user_reply(self, state: PipelineState) -> PhaseResult:
        ...
        # v1.4: 无效输入 → 重新显示提示（不再静默选 [3]）
        if reply not in decision_map:
            prompt = self._format_gap_prompt(gap_result)
            return PhaseResult(
                status="need_input",
                data={"gap_analysis": gap_result},
                prompt=prompt,
                input_type="gap_decision",
            )
        decision = decision_map[reply]
