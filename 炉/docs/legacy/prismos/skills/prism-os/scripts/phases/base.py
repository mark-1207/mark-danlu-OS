...
class PipelineState:
    ...
    # M1: 人设字段（CLI 显式传入优先于 persona）
    identity_role: str = ""
    audience: str = ""
    # M2: 深度模式缓存
    deep_titles: List[dict] = field(default_factory=list)
    ...
    # v1.1: 方向选择（DirectionSelectPhase 写入）
    direction_selected: str = ""
...
class PipelineConfig:
    ...
    # M1: 人设持久化（默认 "default"，对应 data/personas.yaml 中 default 段）
...
class PrismPipeline:
    ...
    def run(self, thesis: str, resume_state: PipelineState = None) -> PipelineState:
        ...
        for i in range(self.state.current_phase_index, len(phases)):
            ...
            # dry_run：每个 Phase 后暂停让用户看输出（--no-interactive 时跳过）
            if self.config.dry_run and self.config.interactive and result.status not in ("need_input", "rejected"):
                import sys
