...
**状态**：v1.1 完整交付 + LLM 优化 + UX 增强

#### v1.1 — 六模块 + 方向选择

**核心**：把 v1.0 的"单次 run 出 1 篇文章"升级为"锚点 + 系列 + 风格连贯 + 密度控制"的完整创作流。

**新增模块（v1.1.0, 2026-06-11）**：
- **M3b 系列模式**：5 维 LLM 适配性分析 + 硬规则覆盖 + 3 标题生成 + nanoid series_id + 冲突豁免
- **M4 A1 张力评分**：LLM 自评 1-5 分 + A1×0.6+A3×0.4 排序权重
- **M5 A2 维度覆盖**：4 维/5 原型覆盖分析 + 命令矩阵 + `r` 补生成
- **M6 A3 风格连贯性**：embedding 余弦相似度 + `storage.append_log` 存 `_title_embedding`
- **M9 C1 风格指纹**：persona → StyleFingerprint → quality_check L5 规则检查
- **C2 密度控制**：定量实体计数 + 定性 LLM 2 项检查 + quality_check L6 集成

**Phase 1.5 方向选择（v1.1.2, 2026-06-11）**：
- 网关 pass 后**总是**展示 2-3 个切入方向让用户选（不只是 clarify 路径）
- 修复消失的"方向选择"流程

**集成**：
- `prism_os.py` 新增 `--series` / `--series-action <yes|no|save-all>` 标志 + `mark_written` 子命令
- `prism_engine.py` + `title_deep.py` 加 tension_score prompt + 解析
- `phases/prism.py` sort_by_score 排序 + 覆盖报告展示 + `r` 补生成
- `content_generator.py` quality_check 加 L5 风格 + L6 密度（persona 触发）

**测试**：229 个全过（v1.1 阶段完成时）

#### LLM 调用优化（v1.1.1, 2026-06-12）

**核心**：30 → ~12 calls per run，~60% 降幅，不牺牲质量。

**B1 P0（零风险）**：
- 删 `generate_prenarrative` 调用（标准 pipeline 不显示）
- 删 expansion 块（靠 prompt 2500 字约束 + writing-cn scene 的 16384 max_tokens）

**B2 P1（低风险）**：
- Twin learn + filter 合并为 `learn_and_filter_combined`
- Prism 4 维 → 2 组（challenges: reversal+contrarian / grounded: benefit_anchor+micro_scene）

**B3 P2（中风险含 3 层防御）**：
- CCOS Layer 4 (模块流) + Layer 8 (势能曲线) 合并为 `generate_module_flow_and_energy`，fallback 容错
- Logic 12→1 合并（`_audit_batch_single` 1 次 LLM 返回数组）：
  1. 校验：返回数组长度 != N → 标记可疑
  2. 重试：长度不匹配时 retry 1 次，prompt 加 "必须返回恰好 N 项"
  3. fallback：retry 仍失败 → 对缺失项逐个 `audit_title`（最坏退化 ~3-4 calls）

#### 决策点 UX 增强（v1.1.2, 2026-06-12）

**核心**：用户不再盲选。⭐ 推荐 + 💡 差异说明（1 次 LLM）。

**决策点 1.5 方向选择**：
- 每方向加 1 次 LLM 生成的差异说明（~30 字）
- 第一个方向默认 ⭐ 推荐
- 跳过时（skip）不阻断，继续用原命题

**决策点 1 Prism 标题**：
- 解除 rationale 50 字截断（显示完整理由）
- 第一个 ⭐ 推荐（sort_by_score 最高）

**决策点 1 deep 模式**：
- 加 archetype 中文标签 + 描述（基于 `title_deep.VALID_BASED_ON` 8 个标准角度）
- 第一个 ⭐ 推荐

**决策点 3 Gap**：
- ⭐ 推荐规则：readiness ≥ 70% → [3] 直接生成，< 70% → [2] 调整大纲
