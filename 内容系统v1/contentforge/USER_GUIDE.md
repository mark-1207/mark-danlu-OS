...
运行时，系统会先问你一个关键问题：**"你想要从什么角度展开？"**

- **🎯 观点输出** — 你对这事有明确看法（比如"远程办公才是未来"）。系统会先用 AI 锤炼你的观点（HKR 质检+证伪），再生成文章。
- **🔍 探索生成** — 你还没想清楚，让 AI 帮你分析展开。走标准 create 流程。

也可以用 `--opinion` 参数直接跳过这个问题，直接走观点路径：

```bash
contentforge create --keyword "远程办公才是未来" --opinion
```
...
你给它一个关键词，它生成一篇 200-500 字的短文，适合**头条、X、社交平台**等短内容平台。

风格要求：**接地气、不说教、金句洞察、口语化**。文章要像跟朋友聊天，不像写报告。

```bash
contentforge create --keyword "AI取代工作" --short
```

### 模式四：观点输出（opinion）

你给它一个观点（比如"远程办公才是未来"），系统会：

1. 用 AI 锤炼你的观点（HKR 质检+证伪+推荐标题）
2. 确认标题和论点
3. 进入标准 create 流程生成文章

```bash
contentforge opinion --keyword "远程办公才是未来"
# 等同于 contentforge create -k "远程办公才是未来" --opinion
```
...
| `create --keyword "AI取代工作" --short` | 短文子模式（200-500字，头条/X/社交平台） |
| `create --keyword "远程办公才是未来" --opinion` | 观点路径（HKR 质检+证伪后生成） |

> **交互式确认说明**：默认情况下，`create` 会先展示 AI 的选题思路（子话题、角度、标题候选），你可以修改后再继续生成。按 `?` 查看 TUI 操作键位。
>
> **观点 vs 探索**：运行 `create` 时会先问"🎯 观点输出 / 🔍 探索生成"。选"观点"走 OpinionRefineStep，选"探索"走标准流程。也可以用 `--opinion` 参数直接跳过这个问题。
...
```bash
contentforge skill "帮我写一篇关于AI的文章 发公众号"
contentforge skill "远程办公才是未来"          # 会走 disambiguation TUI
```

> **注意**：`skill` 不再自动检测 opinion 类型。所有非 recreate 的输入统一走 create 路径，由交互式 disambiguation 决定是否走观点模式。
...
│   ├── {文章标题}.xhs.md                 # 小红书文章
│   └── {文章标题}.douyin.md             # 抖音文案
├── create_short_1780368074779/           # 短文生成 (--short)
│   └── {文章标题}.short.md               # 短文（200-500字）
...
### Q：短文模式（--short）和普通 create 有什么区别？

短文模式跳过了 topic-assignment、outline、material-search、per-platform content/review 这些步骤，直接从 topic-analysis → short-angle-selection → short-content → short-review，4 步串行完成，生成 200-500 字的短文。风格要求：接地气、不说教、金句洞察、口语化。

### Q：观点模式（--opinion）和普通 create 有什么区别？

观点模式在 topic-analysis 之后、topic-assignment 之前，会运行 OpinionRefineStep（HKR 质检+证伪+锤炼论点），把你的观点打磨成一个有论据支撑、有标题推荐的 RefinedOpinion，然后继续标准 create 流程。

### Q：skill 命令还能自动判断 opinion 吗？

不能了。`skill` 现在统一走 create 路径。如果你输入的是观点性内容（比如"远程办公才是未来"），系统会在交互模式下问你"🎯 观点输出 / 🔍 探索生成"，由你决定走哪条路。也可以用 `contentforge opinion` 命令直接走观点路径。
...
| `node dist/index.js create --keyword "主题" --short` | 短文子模式（200-500字） |
| `node dist/index.js create --keyword "观点" --opinion` | 观点路径（HKR 质检后生成） |
| `node dist/index.js opinion --keyword "观点"` | 观点输出（向后兼容别名） |
| `node dist/index.js skill "自然语言描述"` | 自然语言入口（自动判断 create/recreate） |
