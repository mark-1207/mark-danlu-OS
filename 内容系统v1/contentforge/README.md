...
ContentForge 是一个**智能内容生成引擎**，你给它一个关键词，它就能自动生成适配**微信公众号、小红书、抖音**三个平台的高质量文章。也可以输入一篇爆款文章，让 AI 为你生成差异化的二创版本。还支持**短文生成**（200-500字，头条/X/社交平台）和**观点输出**（从个人观点出发锤炼成文）。
...
**输入**：一个关键词
**输出**：200-500字的短文（适合头条、X、社交平台）

风格要求：接地气、不说教、金句洞察、口语化

```bash
contentforge create --keyword "AI取代工作" --short
```

流程：`topic-analysis → short-angle-selection → short-content → short-review`（4步串行，跳过 topic-assignment/outline/material-search/per-platform content/review）

### 场景 D：观点输出 (--opinion)

**输入**：一个观点或主题（如"远程办公才是未来"）
**输出**：经过 HKR 质检锤炼后的观点文章
...
**交互模式**：运行 `create` 时，系统会先问"🎯 观点输出 / 🔍 探索生成"。选"观点"后走 OpinionRefineStep（HKR 质检+证伪+锤炼论点），再进入标准 create 流程。选"探索"则走标准 create 流程。

**skill 命令**：`skill` 不再自动检测 opinion，统一走 create 路径，由交互式 disambiguation 决定是否走观点模式。
...
contentforge create --keyword "AI取代工作" --short   # 短文子模式（200-500字）
contentforge create --keyword "远程办公才是未来" --opinion  # 观点路径
```
...
contentforge skill "远程办公才是未来"          # 会走 disambiguation TUI，问你是观点还是探索
```
...
│   ├── cli/
│   │   ├── commands/              # 命令行命令定义
│   │   │   ├── create.ts          # 原创生成（含 --short/--opinion）
│   │   │   ├── opinion.ts         # 观点输出别名
│   │   │   ├── skill.ts           # 自然语言入口
│   │   │   └── ...
│   │   └── ui/
│   │       ├── disambiguation.ts  # 观点/探索交互式选择
│   │       ├── outline-review.ts  # 大纲确认 TUI
│   │       ├── topic-review.ts    # 选题确认 TUI
│   │       └── ...
│   ├── core/                      # 核心引擎（Pipeline、Step、Context）
│   ├── scenarios/
│   │   ├── create/               # 原创生成场景
│   │   │   ├── index.ts          # buildCreatePipeline + buildShortPipeline
│   │   │   ├── steps/
│   │   │   │   ├── short-angle-selection.ts  # 短文角度选择
│   │   │   │   ├── short-content.ts          # 短文生成
│   │   │   │   ├── short-review.ts           # 短文审查
│   │   │   │   └── ...
│   │   │   └── types.ts          # 含 ShortAngle/ShortContent/ShortReview Schema
│   │   ├── recreate/             # 爆款二创场景
│   │   └── opinion/              # 观点场景（HKR 质检+证伪）
│   │       ├── steps/opinion-refine.ts
│   │       └── types.ts
│   ├── llm/                      # AI 模型适配层
│   ├── prompts/templates/create/ # 含 6 个 short-* prompt 模板
│   └── utils/
├── tests/                         # 单元测试（364/364 通过）
