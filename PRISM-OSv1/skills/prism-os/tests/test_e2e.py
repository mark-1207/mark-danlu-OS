...
class TestE2ECCOSFlow:
    ...
    def test_full_flow_with_mock(self):
        ...
        with patch("cognitive_outline._call_llm_raw", side_effect=_mock_call_llm):
            ...
            # Phase 1-3 执行
            assert result.get("phase") in ["prism", "gap", "complete"], f"Phase 执行异常: {result.get('phase')}"
    ...
    def test_gap_analysis_with_thesis_summary(self):
        ...
        with patch("gap_analysis._call_llm_raw", side_effect=_mock_call_llm):
