...
- **交互重构 - 专业模式新流程**：
  - 删除浮窗预览，改为页面内联展示
  - 生成后显示内容区域（平台Tab + 标题 + 正文）
  - "一键优化"变为两列对比布局（原文 | 优化版）
  - 单选版本后支持导出选中内容
  - 生成完成后"爆款制作启动"按钮禁用并显示"内容已生成"
- **Bug修复 - 流式输出不显示**：
  - 修复 `previewResults.length > 0` 条件在流式生成时为空的问题
  - 改为 `(viewMode === 'generating' || previewResults[0])` 显示流式内容
  - 使用 `selectedPlatforms[0]` 获取平台ID显示流式内容
- **Bug修复 - 优化提示词上下文不足**：
  - 传递更丰富的上下文：`{ content, title, 核心议题, 情绪基调, 目标受众, 内容类型, 赛道, 账号人设 }`
- **Bug修复 - 优化后误操作会丢失结果**：
  - 添加 `hasGenerated` 状态追踪是否已生成
  - 按钮在 result/comparing 模式下禁用
